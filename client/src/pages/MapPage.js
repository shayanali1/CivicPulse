...
// Component to handle map theme changes
        ...
        <header className="flex-shrink-0 bg-white/90 dark:bg-[#0b1326]/90 border-b border-gray-200 dark:border-[#424754] backdrop-blur-md z-50">
          <nav className="flex justify-between items-center px-4 py-3">
            ...
            <div className="flex items-center gap-3">
              ...
              >
                <span className="material-symbols-outlined text-gray-600 dark:text-[#c2c6d6]">
              ...
              {user ? (
                <>
                  ...
                  <div className="flex items-center gap-2 ml-1">
                    ...
                    <button onClick={handleLogout} className="text-xs text-gray-500 dark:text-[#c2c6d6] hover:text-red-500 transition-colors hidden sm:block">
                      Logout
                    </button>
                  </div>
                </>
              ) : (
                <Link to="/login" className="bg-blue-500 text-white px-3 py-2 rounded-lg text-sm font-bold hover:brightness-110 transition-all">
                  Login
                </Link>
              )}
            </div>
          ...
          {sidebarOpen && (
            <aside className="w-80 flex-shrink-0 bg-white dark:bg-[#0f1729] border-r border-gray-200 dark:border-[#424754] flex flex-col overflow-hidden z-40">
              ...
              <div className="p-4 border-b border-gray-200 dark:border-[#424754]">
                <h3 className="text-sm font-bold text-gray-900 dark:text-[#dae2fd] mb-3" style={{ fontFamily: 'Hanken Grotesk' }}>
                  Filter by Category
                </h3>
                <div className="flex flex-wrap gap-2">
                  <button
                    onClick={() => setSelectedCategory('all')}
                    className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${selectedCategory === 'all' ? 'bg-blue-500 text-white' : 'bg-gray-100 dark:bg-[#1e293b] text-gray-600 dark:text-[#c2c6d6] hover:bg-gray-200 dark:hover:bg-[#2d3449]'}`}
                  >
                    All
                  </button>
                  {Object.entries(CATEGORY_CONFIG).map(([key, val]) => (
                    <button
                      ...
                      className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${selectedCategory === key ? 'text-white' : 'bg-gray-100 dark:bg-[#1e293b] text-gray-600 dark:text-[#c2c6d6] hover:bg-gray-200'}`}
                ...
                <h3 className="text-sm font-bold text-gray-900 dark:text-[#dae2fd] mb-3 mt-4" style={{ fontFamily: 'Hanken Grotesk' }}>
                  Filter by Status
                </h3>
                <div className="flex flex-wrap gap-2">
                  <button
                    onClick={() => setSelectedStatus('all')}
                    className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${selectedStatus === 'all' ? 'bg-blue-500 text-white' : 'bg-gray-100 dark:bg-[#1e293b] text-gray-600 dark:text-[#c2c6d6]'}`}
                  >
                    All
                  </button>
                ...
                {loading ? (
                  ...
                  filteredIssues.map((issue) => (
                    <div
                      ...
                      className={`p-3 rounded-xl border cursor-pointer transition-all ${selectedIssue?.id === issue.id ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-200 dark:border-[#424754] bg-white dark:bg-[#171f33] hover:border-blue-300 dark:hover:border-blue-700'}`}
                    >
