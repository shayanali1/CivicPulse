...
export default function OfficialRegisterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.secretCode !== 'GOV-CIVIC-2026') {
      return setError('Invalid authorization code');
    }
    ...
    setLoading(true);
    try {
      const res = await axios.post('http://localhost:5000/api/auth/register-official', {
        name: formData.name,
  ...
  return (
    <div className={isDark ? 'dark' : ''}>
      <div className="min-h-screen bg-white dark:bg-[#0b1326] flex items-center justify-center px-4 py-12 transition-colors duration-300">
        ...
        <button onClick={toggleTheme} className="fixed top-4 right-4 p-2 rounded-lg border border-gray-200 dark:border-[#424754] bg-white dark:bg-[#171f33] hover:bg-gray-100 dark:hover:bg-[#1e293b] transition-colors">
        ...
        <div className="w-full max-w-md">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-500 dark:text-[#c2c6d6] mt-1">Government staff only — authorization required</p>
          </div>
          ...
          <div className="bg-white dark:bg-[#171f33] border border-gray-200 dark:border-[#424754] rounded-2xl p-8 shadow-sm">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-[#c2c6d6] mb-2">Full Name</label>
                <div className="relative">
                  ...
                  <input type="text" name="name" value={formData.name} onChange={handleChange} required
                    ...
                    className="w-full bg-gray-50 dark:bg-[#0b1326] border border-gray-200 dark:border-[#424754] rounded-xl pl-10 pr-4 py-3 text-gray-900 dark:text-[#dae2fd] placeholder-gray-400 focus:border-blue-500 outline-none transition-colors" />
                </div>
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-[#c2c6d6] mb-2">Official Email</label>
                <div className="relative">
                  ...
                  <input type="email" name="email" value={formData.email} onChange={handleChange} required
                    placeholder="official@gov.pk"
                    className="w-full bg-gray-50 dark:bg-[#0b1326] border border-gray-200 dark:border-[#424754] rounded-xl pl-10 pr-4 py-3 text-gray-900 dark:text-[#dae2fd] placeholder-gray-400 focus:border-blue-500 outline-none transition-colors" />
                </div>
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-[#c2c6d6] mb-2">Password</label>
                <div className="relative">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-[#c2c6d6] mb-2">Confirm Password</label>
                <div className="relative">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-[#c2c6d6] mb-2">Authorization Code</label>
                <div className="relative">
                  <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-[20px]">key</span>
                  <input type="password" name="secretCode" value={formData.secretCode} onChange={handleChange} required
                    placeholder="Enter your authorization code"
                    className="w-full bg-gray-50 dark:bg-[#0b1326] border border-gray-200 dark:border-[#424754] rounded-xl pl-10 pr-4 py-3 text-gray-900 dark:text-[#dae2fd] placeholder-gray-400 focus:border-blue-500 outline-none transition-colors" />
                </div>
                <p className="text-xs text-gray-400 mt-1">Contact your district administrator for the authorization code</p>
              </div>

              <button type="submit" disabled={loading}
                className="w-full bg-blue-500 text-white py-3 rounded-xl font-bold text-base hover:brightness-110 active:scale-95 transition-all disabled:opacity-50 flex items-center justify-center gap-2">
                {loading ? (
                  <><span className="material-symbols-outlined animate-spin text-[20px]">progress_activity</span>Creating account...</>
                ) : (
                  <><span className="material-symbols-outlined text-[20px]">shield</span>Create Official Account</>
                )}
              </button>
            </form>
