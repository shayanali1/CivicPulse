...
export default function DashboardPage() {
  ...
  const stats = {
    total: issues.length,
    pending: issues.filter(i => i.status === 'submitted' || i.status === 'under_review').length,
    escalated: issues.filter(i => i.status === 'escalated_l1' || i.status === 'escalated_l2').length,
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-[#0b1326] flex transition-colors duration-300">
        ...
        <aside className="w-64 flex-shrink-0 bg-white dark:bg-[#0f1729] border-r border-gray-200 dark:border-[#424754] flex flex-col fixed h-full z-40">
          ...
          <div className="p-6 border-b border-gray-200 dark:border-[#424754]">
            <Link to="/" className="flex items-center gap-3">
              ...
              <span className="font-bold text-blue-500 text-lg" style={{ fontFamily: 'Hanken Grotesk' }}>CivicPulse</span>
            </Link>
            <div className="mt-3 flex items-center gap-2">
              ...
              <div>
                <p className="text-xs font-medium text-gray-900 dark:text-[#dae2fd]">{user?.name}</p>
            ...
            {[
              ...
              >
                ...
                {item.id === 'escalated' && stats.escalated > 0 && (
                  <span className="ml-auto bg-red-500 text-white text-xs px-2 py-0.5 rounded-full">{stats.escalated}</span>
                )}
              </button>
            ))}
        ...
        <main className="flex-1 ml-64 overflow-auto">
          ...
          <header className="bg-white/90 dark:bg-[#0b1326]/90 border-b border-gray-200 dark:border-[#424754] backdrop-blur-md sticky top-0 z-30 px-8 py-4 flex justify-between items-center">
            <div>
              <h1 className="text-xl font-bold text-gray-900 dark:text-[#dae2fd]" style={{ fontFamily: 'Hanken Grotesk' }}>
            ...
            <button onClick={fetchIssues} className="flex items-center gap-2 px-4 py-2 border border-gray-200 dark:border-[#424754] rounded-xl text-sm text-gray-600 dark:text-[#c2c6d6] hover:bg-gray-100 dark:hover:bg-[#1e293b] transition-all">
            ...
            {activeNav === 'overview' && (
              <>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                  {[
                    { label: 'Total Issues', value: stats.total, icon: 'summarize', color: 'text-blue-500', bg: 'bg-blue-50 dark:bg-blue-900/20' },
                    { label: 'Pending', value: stats.pending, icon: 'pending', color: 'text-yellow-500', bg: 'bg-yellow-50 dark:bg-yellow-900/20' },
                    { label: 'Escalated', value: stats.escalated, icon: 'warning', color: 'text-red-500', bg: 'bg-red-50 dark:bg-red-900/20' },
                    { label: 'Resolved', value: stats.resolved, icon: 'check_circle', color: 'text-green-500', bg: 'bg-green-50 dark:bg-green-900/20' },
                  ].map((stat) => (
                    <div key={stat.label} className="bg-white dark:bg-[#171f33] border border-gray-200 dark:border-[#424754] rounded-2xl p-6 shadow-sm">
                      ...
                      <div className="text-3xl font-bold text-gray-900 dark:text-[#dae2fd] mb-1" style={{ fontFamily: 'Hanken Grotesk' }}>{stat.value}</div>
                      <div className="text-sm text-gray-500 dark:text-[#c2c6d6]">{stat.label}</div>
                    </div>
                  ))}
                </div>
                  ...
                  <div className="space-y-4">
                    {categoryCounts.map((cat) => (
                      <div key={cat.key} className="flex items-center gap-4">
                        <div className="w-24 text-sm text-gray-600 dark:text-[#c2c6d6]">{cat.label}</div>
                        <div className="flex-1 bg-gray-100 dark:bg-[#0b1326] rounded-full h-3">
                          <div
                            className="h-3 rounded-full transition-all duration-500"
                            style={{
                              width: issues.length > 0 ? `${(cat.count / issues.length) * 100}%` : '0%',
                ...
                <div className="bg-white dark:bg-[#171f33] border border-gray-200 dark:border-[#424754] rounded-2xl p-6 shadow-sm">
                  <div className="flex justify-between items-center mb-6">
                    <h2 className="text-lg font-bold text-gray-900 dark:text-[#dae2fd]" style={{ fontFamily: 'Hanken Grotesk' }}>Recent Issues</h2>
                    <button onClick={() => setActiveNav('issues')} className="text-sm text-blue-500 hover:underline">View all</button>
                  </div>
                  <IssuesTable issues={issues.slice(0, 5)} formatDate={formatDate} />
                </div>
              </>
            )}

            {/* ISSUES TAB */}
            {(activeNav === 'issues' || activeNav === 'escalated' || activeNav === 'resolved') && (
              <div className="bg-white dark:bg-[#171f33] border border-gray-200 dark:border-[#424754] rounded-2xl p-6 shadow-sm">
                {/* Filters */}
                ...
                {loading ? (
                  ...
                  <IssuesTable
                    issues={activeNav === 'escalated'
                      ? filteredIssues.filter(i => i.status === 'escalated_l1' || i.status === 'escalated_l2')
                      : activeNav === 'resolved'
...
function IssuesTable({ issues, formatDate }) {
  if (issues.length === 0) {
    return (
      <div className="text-center py-12">
        <span className="material-symbols-outlined text-gray-300 dark:text-[#424754] text-[48px] block mb-3">inbox</span>
      ...
      <table className="w-full">
        <thead>
          <tr className="border-b border-gray-200 dark:border-[#424754]">
            {['Title', 'Category', 'Status', 'Upvotes', 'Date', 'Action'].map((h) => (
              <th key={h} className="text-left text-xs font-semibold text-gray-500 dark:text-[#c2c6d6] uppercase tracking-wider pb-3 pr-4">{h}</th>
            ))}
          </tr>
        </thead>
        <tbody className="divide-y divide-gray-100 dark:divide-[#424754]">
          {issues.map((issue) => {
            ...
            return (
              <tr key={issue.id} className="hover:bg-gray-50 dark:hover:bg-[#0b1326] transition-colors">
                <td className="py-4 pr-4">
                  <p className="text-sm font-medium text-gray-900 dark:text-[#dae2fd] truncate max-w-[200px]">{issue.title}</p>
                </td>
                <td className="py-4 pr-4">
                  <div className="flex items-center gap-2">
                    <span className="material-symbols-outlined text-[16px]" style={{ color: cat.color }}>{cat.icon}</span>
                    <span className="text-sm text-gray-600 dark:text-[#c2c6d6]">{cat.label}</span>
                  </div>
                </td>
                <td className="py-4 pr-4">
                  <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${stat.bg} ${stat.textColor}`}>
                    <span className={`h-1.5 w-1.5 rounded-full ${stat.color}`}></span>
                    {stat.label}
                  </span>
                </td>
                <td className="py-4 pr-4">
                ...
                <td className="py-4 pr-4">
                  <span className="text-sm text-gray-500 dark:text-[#c2c6d6]">{formatDate(issue.created_at)}</span>
                </td>
                <td className="py-4">
                  <Link
                    to={`/issues/${issue.id}`}
